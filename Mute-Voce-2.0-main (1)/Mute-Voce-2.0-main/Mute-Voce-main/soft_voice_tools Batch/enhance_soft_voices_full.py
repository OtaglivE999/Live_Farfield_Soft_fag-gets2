...
input_path = (
    input("Enter full path to your audio/video file (.wav, .mp3, .mp4): ")
    .strip()
    .strip('"')
)
...
try:
    ...
    gain_expanded = (
        gain_expanded[: len(y)]
        if len(gain_expanded) > len(y)
        else np.pad(gain_expanded, (0, len(y) - len(gain_expanded)))
    )
...
if ext == ".mp4":
    try:
        ...
        cmd = (
            f'ffmpeg -y -i "{input_path}" -i "{wav_output}" -c:v copy '
            f'-map 0:v:0 -map 1:a:0 -shortest "{mp4_output}"'
        )
